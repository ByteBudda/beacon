...
async function api(path, method = "GET", body) {
    ...
    // Remove /v1 prefix if present (API already includes /api/v1)
    if (path.startsWith("/v1/")) {
        path = path.substring(3);  // Remove /v1, keep rest like /promocodes
    }
...
async function register() {
    ...
    const r = await api("/v1/auth/register", "POST", { username, email, password: pw, referral_code: refCode });
...
async function forgotPassword() {
    ...
    await api("/v1/auth/password-reset", "POST", { email });
...
async function loadFolders() {
    const r = await api("/v1/links/folders");
...
async function createFolder() {
    ...
    const r = await api("/v1/links/folders", "POST", { name, color });
...
async function bulkCreate() {
    ...
    for (const url of urls) {
        const r = await api("/v1/links", "POST", { url, folder_id: folderIdNum });
...
async function changePassword() {
    ...
    const r = await api("/v1/auth/change-password", "POST", { old_password: oldPw, new_password: newPw });
    if (r?.error) { alert(err(r)); return; }
...
async function changeEmail() {
    ...
    if (!password) return;
    const r = await api("/v1/account/email", "PUT", { new_email: newEmail, password });
    if (r?.error) { alert(err(r)); return; }
    user.email = newEmail;
...
async function deleteAccount() {
    ...
    const r = await api("/v1/account", "DELETE", { password });
    if (r?.error) { alert(err(r)); return; }
...
async function redeemPromocode(tab = 'account') {
    const inputId = tab === 'plans' ? 'promocode-input-plans' : 'promocode-input';
    const resultId = tab === 'plans' ? 'promocode-result-plans' : 'promocode-result';
    const code = document.getElementById(inputId).value.trim().toUpperCase();
    if (!code) { alert("Введите промокод"); return; }
    const resultEl = document.getElementById(resultId);
    resultEl.style.display = "none";
    
    const btnId = tab === 'plans' ? 'btn-redeem-promo-tab' : 'btn-redeem-promo';
...
async function resendVerification(email) {
    if (!email) { alert("Введите email"); return; }
    const r = await api("/v1/auth/resend-verification", "POST", { email });
...
async function shortenUrl() {
    ...
    let r;
    if (token) {
        r = await api("/v1/links", "POST", body);
        ...
        r = await api("/v1/links/anonymous", "POST", body);
    }
...
async function dashShortenUrl() {
    ...
    if (pw) { body.is_password_protected = true; body.password = pw; }
    const r = await api("/v1/links", "POST", body);
    btn.disabled = false; btn.textContent = "Сократить";
    if (r?.id) {
        const sUrl = `${location.origin}/s/${r.slug}`;
...
async function loadPlans() {
    const r = await api("/v1/payments/plans");
    if (r?.error) return;
    const grid = document.getElementById("plans-grid");
    grid.innerHTML = "";
    const sub = await api("/v1/payments/subscription/current");
    const cur = sub?.plan || user?.plan || "free";
    const autoRenew = sub?.auto_renew || false;
    
    let planInfo = `Ваш тариф: <strong>${({ free: "Free", pro: "Pro", business: "Business" })[cur]}</strong>`;
    if (autoRenew && cur !== "free") {
        planInfo += ` <button class="btn-ghost" onclick="cancelSubscription()" style="margin-left:12px;padding:4px 12px;font-size:12px;">Отменить подписку</button>`;
    }
    document.getElementById("current-plan-info").innerHTML = planInfo;
...
async function cancelSubscription() {
    if (!confirm("Вы уверены, что хотите отменить подписку? До конца периода вы сохраните доступ к платным функциям.")) return;
    
    const r = await api("/v1/payments/subscription/cancel", "POST");
    if (r?.error) {
        alert("Ошибка: " + err(r));
        return;
    }
    
    alert(r.message || "Подписка отменена");
    loadPlans();
}

async function loadReferral() {
    const r = await api("/v1/auth/referral");
    if (r?.error) return;
...
async function pay(planId) {
    ...
    const r = await api("/v1/payments", "POST", { plan: planId, provider: prov });
    if (r?.error) { alert("Ошибка: " + err(r)); return; }
    if (r.confirmation_url?.includes("/demo/")) {
        const c = await api(`/payments/demo/${r.payment_id}`);
        if (!c?.error) { alert("Тариф активирован!"); const p = await api("/v1/auth/profile"); if (p?.id) user = p; loadPlans(); }
    } else if (r.confirmation_url) window.open(r.confirmation_url, "_blank");
}

async function loadPayments() {
    const r = await api("/v1/payments");
...
async function adminEdit(id) {
    ...
    const input = prompt(`Изменение тарифа\nФормат: тариф.дней (например: pro.30)\nТекущий: ${r.plan}${currentSub}`, r.plan + ".30");
    if (input === null) return;
    
    const parts = input.trim().split('.');
    const newPlan = parts[0].toLowerCase();
    const days = parseInt(parts[1]) || 30;
    
    if (!['free', 'pro', 'business'].includes(newPlan)) {
        alert('Неверный тариф. Используйте: free, pro или business');
        return;
    }
    
    // Extend first (creates subscription if not exists), then set plan
    if (newPlan !== 'free' && days > 0) {
        const ext = await api(`/admin/users/${id}/extend`, "POST", { days: days, plan: newPlan });
        if (ext?.error) {
            alert('Ошибка: ' + err(ext));
            return;
        }
    }
    
    await api(`/admin/users/${id}`, "PUT", { plan: newPlan });
    
    if (newPlan !== 'free' && days > 0) {
        alert(`Тариф: ${newPlan}, продлён на ${days} дней`);
    } else {
        alert(`Тариф изменён на: ${newPlan}`);
    }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // btn-redeem-promo and btn-redeem-promo-tab use onclick in HTML
    ...
    if (p.get("reset")) {
        ...
        if (pw) api("/v1/auth/password-reset-confirm", "POST", { token: p.get("reset"), new_password: pw })
