...
async def update_settings(data: dict, request: Request, admin: dict = Depends(require_admin)):
    ...
    await db.commit()
    logger.info(f"Admin updated settings: {list(data.keys())}")
...
async def extend_user_subscription(
    ...
    cur = await db.execute(
        "SELECT expires_at FROM subscriptions WHERE user_id = ? AND status = 'active' ORDER BY created_at DESC LIMIT 1",
        (user_id,)
    )
    row = await cur.fetchone()
    
    current_expires = row[0] if row else now
    new_expires = max(current_expires, now) + days * 86400
    
    # Update subscription
    await db.execute(
        "UPDATE subscriptions SET expires_at = ?, updated_at = ? WHERE user_id = ? AND status = 'active'",
