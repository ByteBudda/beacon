...
class PaymentService:
    ...
    async def cancel_subscription(self, user_id: int, db) -> dict:
        """Cancel subscription - keep remaining days, stop auto-renew"""
        import time
        ...
        sub_id, plan, starts_at, expires_at, auto_renew = row
        
        if not auto_renew:
            return {"error": "Subscription is not set to auto-renew"}
        
        # Calculate remaining days
        now = time.time()
        remaining_days = 0
        if expires_at and expires_at > now:
            remaining_days = int((expires_at - now) / 86400)
        
        # Disable auto-renew (subscription will expire normally)
        await db.execute(
            "UPDATE subscriptions SET auto_renew = 0, updated_at = ? WHERE id = ?",
            (now, sub_id)
        )
        
        await db.commit()
        
        logger.info(f"User {user_id} canceled subscription. Remaining days: {remaining_days}")
        ...
        return {
            ...
            "expires_at": datetime.fromtimestamp(expires_at) if expires_at else None,
            "message": f"Подписка отменена. Доступ до {remaining_days} дней"
        }
